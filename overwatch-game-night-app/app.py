...
def api_balance_teams():
    ...
    try:
        ...
        if len(selected_players) < min_players:
            return jsonify({"error": f"Need at least {min_players} players for {team_format} team balancing"}), 400
        ...
        return jsonify({
            ...
            "metrics": {
                "avg_comfort": float(np.mean(comfs)),
                "sr_difference": float(abs(avg_srs[0] - avg_srs[1]) if len(avg_srs) >= 2 else 0),
...
def optimize_team_matching(df, team_format='5v5'):
    ...
    cost -= WEIGHTS["SR_DIFF_PENALTY_WEIGHT"] * (cp.max(team_avg_srs) - cp.min(team_avg_srs))
    ...
    result_df = pd.DataFrame({
        ...
        "Role": [["Tank", "DPS", "Support"][roles[i]] if roles[i] != -1 else "Bench" for i in range(len(roles))],
        ...
        if num_players:
            ...
            for game in games:
                ...
                if 'any' in str(game.get('description', '')).lower():
                    ...
                    import re
                    match = re.search(r'at least (\d+)', str(game.get('description', '')).lower())
            ...
            if suitable_games:
                selected_game = suitable_games[np.random.randint(0, len(suitable_games))]
            else:
...
def api_close_voting():
    ...
    if category in votes and votes[category].get('votes'):
        # Calculate winner
        vote_counts = {player: len(voters) for player, voters in votes[category]['votes'].items()}
...
def api_reset_category():
    ...
    socketio.emit('category_reset', {
        'category': category
    }, room='voting')

    return jsonify({"status": "success", "message": f"Category '{category}' has been reset"})
...
def on_nominate_player(data):
    ...
    existing = [n for n in votes[category]['explanations'][nominated_player] if n['user_id'] == user_id]
...
def on_nominate_custom_category(data):
    ...
    # Add nomination if not already nominated by this user
    nomination_entry = {'user_id': user_id, 'explanation': explanation}
    existing = [n for n in votes[custom_category]['explanations'][nominated_player] if n['user_id'] == user_id]

    if not existing:
        votes[custom_category]['nominations'][nominated_player].append(user_id)
        votes[custom_category]['explanations'][nominated_player].append(nomination_entry)

    save_json_file(VOTES_FILE, votes)

    # Broadcast custom category creation/update
    emit('custom_category_update', {
