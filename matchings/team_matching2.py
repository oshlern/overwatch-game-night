...
def optimize_team_matching(role_srs, role_comfs, ids_df):
    ...
    avg_comf = cp.sum(comfs) / id_n  # Maximize average comfortability
    cost = avg_comf  # Use average comfort instead of total comfort
    cost += avg_sr_weight * avg_sr  # Add average SR weight
    cost -= sr_diff_penalty_weight * (cp.max(team_avg_srs) - cp.min(team_avg_srs))  # Penalize large SR differences
...
def plot_summary_data(n_players, identity, role_comfs, role_srs, avg_sr_weight, sr_diff_penalty_weight, id_n, n_roles, team_size, n_teams):
    ...
    for i in range(n_players):
        for j in range(i+1, n_players):
            ...
            swapped_avg_comf = np.sum(swapped_comfs) / id_n  # Calculate average comfort
            swapped_cost = swapped_avg_comf  # Use average comfort instead of total comfort
            swapped_cost += avg_sr_weight * swapped_avg_sr
            swapped_cost -= sr_diff_penalty_weight * (np.max(swapped_team_avg_srs) - np.min(swapped_team_avg_srs))
            ...
            avg_sr_components.append(avg_sr_weight * swapped_avg_sr)
            sr_deviation_components.append(-sr_diff_penalty_weight * (np.max(swapped_team_avg_srs) - np.min(swapped_team_avg_srs)))
    ...
    plt.plot(comfort_components, label=f'Comfort Component (Weight: 1)')
    plt.plot(avg_sr_components, label=f'Average SR Component (Weight: {avg_sr_weight})')
    plt.plot(sr_deviation_components, label=f'SR Deviation Component (Weight: {sr_diff_penalty_weight})')
    plt.xlabel('Swap Pair Index')
